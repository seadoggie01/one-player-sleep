...
execute if entity @a[tag=isSleeping,scores={OPSsleep=1}] run tellraw @a ["",{"selector":"@p"},{"text":" is now sleeping","color":"blue","clickEvent":{"action":"run_command","value":"/trigger OPSkickBed set 1"},"hoverEvent":{"action":"show_text","value":{"text":"","extra":[{"text":"Click to wake!","color":"aqua"}]}}}]
